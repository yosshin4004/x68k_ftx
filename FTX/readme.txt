...
ver. 2.02 : (2022/2/11)
  ...
  ● ftx_palette_set のバグ修正
    引数 color が負の値の時正しく動作しない問題を解消するため、引数 color
    を short 型から unsigned short 型に修正した。

  ● 常時スーパーバイザモードの指定
    FTXlib.s の ALWAYS_SUPER を 1 を指定すると、常にスーパーバイザモード
    から実行されていると見なし、スーパーバイザモードへの切り替えと、
    ユーザーモードへの復帰処理を省略するように修正。
